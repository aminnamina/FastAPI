...
def get_notes(
    ...
    notes = db.exec(select(Note).where(Note.owner_id == current_user.id)).all()
    return [NoteOut(id=note.id, title=note.title, content=note.content, owner_id=note.owner_id) for note in notes]
